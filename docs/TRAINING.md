...
训练使用固定的官方 `Qwen/Qwen3.5-0.8B@2fc06364715b967f1860aea9cf38778875588b17` 完整多模态模型。原始参数全部冻结，只更新语言 LoRA 和 256 维指针头。训练前后对每一个冻结参数计算 SHA-256，校验结果写入模型目录的 `base_integrity.json`。历史 v0.2 通用决策训练和 v0.3.0 贪吃蛇专项续训均已完成；专项模型与评估结果见 [SNAKE_MODEL](SNAKE_MODEL.md)，下文提供复现流程。
