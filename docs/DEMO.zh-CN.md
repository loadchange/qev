...
页面把最近 5 次请求保留在浏览器内存中，用于恢复到编辑器，不会自动重新发送。另有服务端记录：`qev serve` 默认将两个模型 POST 端点的请求和已完成响应保存到 `runs/request-logs`，包含完整内联图片数据和提取的媒体文件。可用 `--request-log-dir PATH` 指定其他目录，或用 `--no-request-log` 关闭服务端记录。响应头 `X-Qev-Request-Id` 和 `X-Qev-Log-Status` 用于定位记录、确认写入状态；日志文件只在本地查看，不通过网页提供。目录结构及异常情况见 [本地请求日志](API.zh-CN.md#本地请求日志)。
